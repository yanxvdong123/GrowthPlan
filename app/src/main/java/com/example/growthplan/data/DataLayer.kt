...
class Converters {
    ...
    @TypeConverter
    fun fromPlanType(value: String): PlanType = PlanType.valueOf(value)
    @TypeConverter
...
@Database(entities = [Plan::class, Task::class], version = 1, exportSchema = false)
    ...
    companion object {
        ...
        fun getDatabase(context: Context): AppDatabase {
            return INSTANCE ?: synchronized(this) {
                val instance = Room.databaseBuilder(context.applicationContext, AppDatabase::class.java, "growth_db").build()
                INSTANCE = instance
                instance
            }
        }
...
class GrowthRepository(private val planDao: PlanDao, private val taskDao: TaskDao) {
    ...
    fun getTasksForToday(): Flow<List<Task>> {
        ...
        return taskDao.getTasksForDate(start, cal.timeInMillis)
    }
